...
def process_and_visualize(image, idx, gts1, gts2, mask, title=None, draw=True, use_estimation=False):
    ...
    if use_estimation:
        ...
        corrected1 = np.where(corrected1 == [0, 0, 0], (gt2 * 255).astype(np.uint8), corrected1)
        corrected2 = np.where(corrected2 == [0, 0, 0], (gt1 * 255).astype(np.uint8), corrected2)
    ...
    if draw:
        pu.visualize([mask, image, corrected, corrected2, corrected1],
                     titles=['a)', 'b)', 'c)', 'd)', 'e)'],
                     in_line=False,
