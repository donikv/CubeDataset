...
def process_with_edges(image, gt, use_edges, use_grad, desaturate, planckian, single):
    ...
    image_cor = iu.color_correct_single(image, c_ill=1/3, u_ill=gt)
    if use_edges:
    ...
    if identation_index < 10:
        ...
        return False, image, None, None, None, mask, None, None
...
def main_process(data):
    ...
    image = iu.process_image(image, 16, blacklevel=0)
    ...
    if succ:
        ...
        if save:
            ...
            name = f'{img_name}-9{"-sing" if single else ""}{"-rand" if not planckian else ""}{"-sat" if not desaturate else ""}{"-grad" if use_grad else ""}{"-edg" if use_edges else ""}.png'
            cv2.imwrite(f'/home/donik/relighted_tau/images/{name}', cv2.cvtColor(relighted, cv2.COLOR_RGB2BGR))
            # cv2.imwrite(f'/home/donik/relighted_tau/corrected/{name}', cv2.cvtColor(image, cv2.COLOR_RGB2BGR))
            cv2.imwrite(f'/home/donik/relighted_tau/gt/{name}', cv2.cvtColor((colored_mask * 255).astype(np.uint8), cv2.COLOR_RGB2BGR))
            cv2.imwrite(f'/home/donik/relighted_tau/gt_mask/{name}',
                        cv2.cvtColor((mask * 255).astype(np.uint8), cv2.COLOR_RGB2BGR))
            cv2.imwrite(f'/home/donik/relighted_tau/img_corrected_1/{name}',
                        cv2.cvtColor(relighted1, cv2.COLOR_RGB2BGR))
            np.savetxt(f'/home/donik/relighted_tau/ill/{name[:-4]}.txt', np.expand_dims(np.concatenate([i1, i2]), axis=0), fmt='%.7f', newline="")
            print(f'Saved {name}')
    else:
        if draw:
...
if __name__ == '__main__':
    ...
    img_folder = '/home/donik/relighted_tau/images/'
    gt_folder = "/home/donik/relighted_tau/gt/"
    gt_mask_folder = "/home/donik/relighted_tau/gt_mask/"
    img_cor_folder = "/home/donik/relighted_tau/img_corrected_1/"
    ill_folder = "/home/donik/relighted_tau/ill/"
    corrected = '/home/donik/relighted_tau/corrected/'
    folders = [img_folder, gt_folder, gt_mask_folder, img_cor_folder, ill_folder, corrected]
